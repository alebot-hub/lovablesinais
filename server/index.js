...
function monitorPrice(symbol, entryPrice, targets, stopLoss, chatId) {
  binance.watchTicker(symbol).then(ticker => {
    ...
    if (currentPrice >= targets[0]) {
      bot.sendMessage(chatId, `${symbol} atingiu TP1: $${targets[0].toFixed(2)}`);
      targets.shift();
...
schedule.scheduleJob('0 * * * *', async () => {
  ...
  if (bestSignal.symbol) {
    ...
    await sendSignal('CHAT_ID', symbol, entryPrice, indicators, patterns, score);
    const targets = Array.from({ length: 6 }, (_, i) => entryPrice * (1 + 0.015 * (i + 1)));
    const stopLoss = entryPrice * (1 - 0.045);
    monitorPrice(symbol, entryPrice, targets, stopLoss, 'CHAT_ID');
  }
});

schedule.scheduleJob('0 */4 * * *', () => analyzeBitcoin('CHAT_ID'));
schedule.scheduleJob('0 */6 * * *', () => analyzeMarketSentiment('CHAT_ID'));
