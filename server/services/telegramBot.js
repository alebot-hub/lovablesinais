...
class TelegramBotService {
  ...
  generateConsistentFactors(signal, isLong) {
    ...
    // Fatores baseados no RSI
    if (signal.indicators?.rsi !== undefined) {
      const rsi = signal.indicators.rsi;
      if (isLong && rsi < 25) {
        factors.push('• RSI em sobrevenda extrema - forte oportunidade de compra');
      } else if (!isLong && rsi > 85) {
        factors.push('• RSI em sobrecompra extrema - forte oportunidade de venda');
      } else if (isLong && rsi < 35) {
        factors.push('• RSI em sobrevenda - oportunidade de compra');
      } else if (!isLong && rsi > 75) {
        factors.push('• RSI em sobrecompra - oportunidade de venda');
      }
    }