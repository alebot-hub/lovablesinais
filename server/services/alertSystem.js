...
class AlertSystemService {
  ...
  async checkMarketConditions(marketData) {
    ...
    // Alerta de performance excepcional
    if (marketData.winRate && marketData.winRate > 85) {
      alerts.push({
        type: 'EXCEPTIONAL_PERFORMANCE',
        message: `🎉 Performance excepcional: ${marketData.winRate}% de acerto!`,
        priority: 'MEDIUM'
      });
    }

    // Alerta de volume anômalo específico por ativo
    if (marketData.topVolumes) {
      marketData.topVolumes.forEach(asset => {
        if (asset.volumeRatio > 3) {
          alerts.push({
            type: 'ASSET_VOLUME_SPIKE',
            message: `🔥 ${asset.symbol}: Volume ${(asset.volumeRatio * 100).toFixed(0)}% acima da média`,
            priority: 'HIGH'
          });
        }
      });
